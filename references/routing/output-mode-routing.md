...
7. 如果问题偏长或多问并行，先回看 [focus-anchor.md](../clarification/focus-anchor.md)，确认主问题、原始问题对照和案件工作单没有漂移。
8. 先做一次 `输出前答题校验`，确认没有漏题、串题或把传闻写成定论。
9. 在澄清已经闭合的前提下，再确认用户要 `深度分析` 还是 `HTML 报告`。
10. 如果用户要 `HTML 报告`，按 `visual-report-spec.md` 在用户当前工作目录输出 HTML，并在关系结构复杂时优先考虑关系图模块。
11. 如果用户要 `深度分析`，用文字方式输出同一套内容骨架。
